...
public class GetCategoryProgressionHandlerTests : IDisposable
{
    ...
    public async Task CalculateCorrectSums()
    {
        ...
        context.AddRange(
            new List<Payment>
            {
                new(DateTime.Today, 60, PaymentType.Income, account, category: category),
                new(DateTime.Today, 20, PaymentType.Expense, account, category: category),
                new(DateTime.Today.AddMonths(-1), 50, PaymentType.Expense, account, category: category),
                new(DateTime.Today.AddMonths(-2), 40, PaymentType.Expense, account, category: category)
            });
        context.Add(account);
