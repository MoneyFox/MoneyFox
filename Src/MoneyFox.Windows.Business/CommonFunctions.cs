        ...
        public static async Task<List<string>> GetPreviouspaymentsAsync(string tilesize, int takeamount = 8)
        {
            ...
            foreach (var item in lastmonth)
            {
                ...
                if (item.Type == Foundation.PaymentType.Income)
                {
                    ...
                    {
                        case "medium":
                            returnlist.Add(item.Chargeaccountname + " +" + LiveTileHelper.TruncateNumber(item.Myamount));
                            break;
                                    ...
                                    Children =
                                    {
                                        ...
                                        new AdaptiveText()
                                            ...
                                            new AdaptiveText()
                                                ...
                                                new AdaptiveText()
                                                {
                                                    Text = string.Format(GetResourceKey("LiveTileLastMonthsExpenses"),System.Globalization.DateTimeFormatInfo.CurrentInfo.GetAbbreviatedMonthName(DateTime.Now.AddMonths(-1).Month), LiveTileHelper.TruncateNumber(GetMonthExpenses(DateTime.Now.AddMonths(-1).Month, DateTime.Now.Year,acct))),
                                                    HintStyle = AdaptiveTextStyle.CaptionSubtle
                                                },
                                               new AdaptiveText()
                                                {
                                                    Text = string.Format(GetResourceKey("LiveTileCurrentMonthsExpenses"),System.Globalization.DateTimeFormatInfo.CurrentInfo.GetAbbreviatedMonthName(DateTime.Now.Month),LiveTileHelper.TruncateNumber(GetMonthExpenses(DateTime.Now.Month,DateTime.Now.Year,acct))),
                                                    HintStyle = AdaptiveTextStyle.CaptionSubtle
                                                }
                                            }
                                        }
